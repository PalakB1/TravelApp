...
const KPIS = [
  { l: "Revenue booked", v: "₹66.3L", f: "+ GST/TCS billed ₹71L", c: "#46e6a8" },
  { l: "Your cost", v: "₹47.3L", f: "hotels · cars · drivers", c: "#7cd6ff" },
  { l: "Profit", v: "₹18.9L", f: "29% margin", c: "#9b8cff" },
  { l: "Outstanding", v: "₹63.4L", f: "due from customers", c: "#ff7a4d" },
];
const BARS = [58, 84, 44, 72, 96, 60, 80];
const STOPS = [
  { x: 24, y: 96, name: "Reykjavík" },
  { x: 268, y: 44, name: "Vík" },
  { x: 512, y: 96, name: "Höfn" },
  { x: 730, y: 52, name: "Egilsstaðir" },
  { x: 968, y: 60, name: "Akureyri" },
];
    ...
    <div className={s.wrap}>
      ...
      <div className={s.contours} aria-hidden>
        <svg viewBox="0 0 1200 760" preserveAspectRatio="none">
          {[90, 170, 250, 330, 410, 490, 570, 650].map((y) => (
            <path key={y} d={contour(y)} fill="none" stroke="rgba(70,230,168,0.5)" strokeWidth="1" />
          ))}
        </svg>
      </div>
          ...
          <svg viewBox="0 0 1000 150" role="img" aria-label="A self-drive route from Reykjavík to Akureyri">
            <defs>
              <linearGradient id="rg" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0" stopColor="#46e6a8" /><stop offset="1" stopColor="#7cd6ff" />
              </linearGradient>
            </defs>
            ...
            {STOPS.map((p) => (
              <g key={p.name}>
                ...
                <text className={s.stopLbl} x={p.x} y={p.y + 26} textAnchor="middle">{p.name}</text>
              </g>
            ))}
          </svg>
        </div>
        ...
        <div className={`${s.visual} ${s.up}`} style={{ animationDelay: "0.34s" }}>
          ...
          <div className={s.console}>
            <div className={s.cbar}>
              <span className={s.cdot} style={{ background: "#ff7a4d" }} />
              <span className={s.cdot} style={{ background: "#f2c94c" }} />
              <span className={s.cdot} style={{ background: "#46e6a8" }} />
